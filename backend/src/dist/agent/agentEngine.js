"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getAgentStatus = exports.runSimulation = exports.executeTrade = exports.stopAgent = exports.startAgent = exports.runAgent = void 0;
const web3_js_1 = require("@solana/web3.js");
const env_1 = require("../config/env");
const riskEngine_1 = require("./riskEngine");
const websocket_1 = require("../websocket");
const decisionEngine_1 = require("./decisionEngine");
const walletManager_1 = require("../wallet/walletManager");
const jupiterDecisionLogic_1 = require("./jupiterDecisionLogic");
const jupiterGovernor_1 = require("./jupiterGovernor");
const jupiterSwapModule_1 = require("../protocols/jupiterSwapModule");
const splEconomyEngine_1 = require("./splEconomyEngine");
const todayKey = () => new Date().toISOString().slice(0, 10);
const runtime = {
    running: false,
    cycleInProgress: false,
    intervalMs: env_1.env.defaultLoopMs,
    timer: null,
    cycles: 0,
    failures: 0,
    lastRunAt: null,
    lastAction: "Agent initialized",
    lastDecision: null,
    lastIntent: null,
    lastTxSignature: null,
    lastExplorerUrl: null,
    topology: null,
    swapStats: {
        lastSwapAt: null,
        successfulSwaps: 0,
        failedSwaps: 0,
        dailySwapCount: 0,
        dailyKey: todayKey(),
        consecutiveSwapFailures: 0,
    },
};
const WSOL_MINT = "So11111111111111111111111111111111111111112";
const USDC_DECIMALS = 6;
const setLastExecution = (result) => {
    runtime.lastAction = result.message;
    runtime.lastTxSignature = result.txSignature ?? null;
    runtime.lastExplorerUrl = result.explorerUrl ?? null;
    runtime.lastRunAt = new Date().toISOString();
};
const resetDailyCountersIfNeeded = () => {
    const key = todayKey();
    if (runtime.swapStats.dailyKey !== key) {
        runtime.swapStats.dailyKey = key;
        runtime.swapStats.dailySwapCount = 0;
    }
};
const ensureTopology = async () => {
    if (runtime.topology) {
        return runtime.topology;
    }
    const previousActive = (0, walletManager_1.getActiveWalletPublicKey)();
    const treasury = await (0, walletManager_1.ensureWalletByLabel)("Treasury Agent");
    const trader = await (0, walletManager_1.ensureWalletByLabel)("Trader Agent");
    const liquidity = await (0, walletManager_1.ensureWalletByLabel)("Liquidity Agent");
    const arbitrage = await (0, walletManager_1.ensureWalletByLabel)("Arbitrage Agent");
    if (previousActive) {
        await (0, walletManager_1.switchWallet)(previousActive).catch(() => undefined);
    }
    runtime.topology = {
        treasuryWalletPublicKey: treasury.publicKey,
        traderWalletPublicKey: trader.publicKey,
        operationalWalletPublicKeys: [
            trader.publicKey,
            liquidity.publicKey,
            arbitrage.publicKey,
        ],
    };
    try {
        await (0, splEconomyEngine_1.initializeSplEconomy)(runtime.topology);
    }
    catch (error) {
        const message = error instanceof Error && error.message.trim()
            ? error.message
            : "SPL economy initialization failed with an unknown error.";
        (0, websocket_1.emitLog)({
            level: "warn",
            message,
        });
    }
    return runtime.topology;
};
const estimateSolPriceUsdc = async () => {
    if (!env_1.env.enableJupiterSwap) {
        return 100;
    }
    try {
        const quote = await jupiterSwapModule_1.JupiterSwapModule.fetchQuote({
            inputMint: WSOL_MINT,
            outputMint: env_1.env.defaultUsdcMint,
            amount: web3_js_1.LAMPORTS_PER_SOL,
            slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        });
        const outAmountRaw = Number(quote.outAmount ?? 0);
        if (!Number.isFinite(outAmountRaw) || outAmountRaw <= 0) {
            return 100;
        }
        return outAmountRaw / 10 ** USDC_DECIMALS;
    }
    catch {
        return 100;
    }
};
const mapIntentToDecision = (intent, amountSol) => {
    if (intent.action === "HOLD" || !intent.direction) {
        return {
            action: "HOLD",
            protocol: "hold",
            amountSol: 0.1,
            slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
            confidence: intent.confidence,
            reason: intent.reason,
        };
    }
    return {
        action: intent.direction === "SOL_TO_USDC" ? "SELL" : "BUY",
        protocol: "jupiter_swap",
        inputMint: intent.direction === "SOL_TO_USDC" ? WSOL_MINT : env_1.env.defaultUsdcMint,
        outputMint: intent.direction === "SOL_TO_USDC" ? env_1.env.defaultUsdcMint : WSOL_MINT,
        amountSol,
        slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        confidence: intent.confidence,
        reason: intent.reason,
    };
};
const buildIntentContext = async () => {
    resetDailyCountersIfNeeded();
    const topology = await ensureTopology();
    const walletState = await (0, walletManager_1.getWalletState)();
    const traderWallet = walletState.wallets.find((wallet) => wallet.publicKey === topology.traderWalletPublicKey);
    if (!traderWallet) {
        throw new Error("Trader wallet is missing from wallet state.");
    }
    const solPriceUsdc = await estimateSolPriceUsdc();
    const totalTrades = runtime.swapStats.successfulSwaps + runtime.swapStats.failedSwaps;
    const successRate = totalTrades === 0 ? 1 : runtime.swapStats.successfulSwaps / totalTrades;
    const intent = (0, jupiterDecisionLogic_1.decideJupiterIntent)({
        solBalance: traderWallet.solBalance,
        usdcBalance: traderWallet.usdcBalance,
        solPriceUsdc,
        lastTradeAt: runtime.swapStats.lastSwapAt,
        tradeSuccessRate: successRate,
        dailyTradeCount: runtime.swapStats.dailySwapCount,
        consecutiveFailures: runtime.swapStats.consecutiveSwapFailures,
        estimatedSlippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        minSolReserve: env_1.env.jupiterMinSolReserve,
    });
    let amountSol = 0;
    let notionalUsdc = 0;
    if (intent.action === "SWAP" && intent.direction === "SOL_TO_USDC") {
        amountSol = Number((traderWallet.solBalance * intent.amountPct).toFixed(6));
        notionalUsdc = amountSol * solPriceUsdc;
    }
    else if (intent.action === "SWAP" && intent.direction === "USDC_TO_SOL") {
        notionalUsdc = traderWallet.usdcBalance * intent.amountPct;
        amountSol = Number((notionalUsdc / Math.max(solPriceUsdc, 0.0001)).toFixed(6));
    }
    return {
        topology,
        traderWallet,
        intent,
        amountSol: Math.max(amountSol, 0),
        notionalUsdc: Math.max(notionalUsdc, 0),
        solPriceUsdc,
        successRate,
    };
};
const runDeterministicCycle = async () => {
    const context = await buildIntentContext();
    const decision = mapIntentToDecision(context.intent, context.amountSol);
    runtime.lastDecision = decision;
    runtime.lastIntent = context.intent;
    (0, websocket_1.emitLog)({
        level: "info",
        message: `Decision: ${context.intent.action} ${context.intent.direction ?? ""}`,
        data: {
            confidence: context.intent.confidence,
            reason: context.intent.reason,
            amountPct: context.intent.amountPct,
        },
    });
    if (context.intent.action === "HOLD") {
        setLastExecution({
            message: `HOLD: ${context.intent.reason}`,
        });
        return (0, exports.getAgentStatus)();
    }
    if (!env_1.env.enableJupiterSwap) {
        const reason = "Jupiter swap is disabled by environment configuration.";
        (0, websocket_1.emitLog)({ level: "warn", message: reason });
        setLastExecution({ message: reason });
        return (0, exports.getAgentStatus)();
    }
    const riskScore = context.intent.confidence < 1
        ? 100
        : Number((runtime.swapStats.consecutiveSwapFailures * 18 +
            (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps / 5 +
            (context.traderWallet.solBalance < env_1.env.jupiterMinSolReserve ? 25 : 0) +
            (1 - context.successRate) * 20).toFixed(2));
    const governor = (0, jupiterGovernor_1.evaluateJupiterGovernor)({
        solBalance: context.traderWallet.solBalance,
        requestedSwapSol: context.amountSol,
        requestedSwapPct: context.intent.amountPct,
        slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        maxAllowedSlippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        dailySwapCount: runtime.swapStats.dailySwapCount,
        riskScore,
    });
    if (!governor.allowed) {
        (0, websocket_1.emitLog)({
            level: "warn",
            message: `Governor rejected swap: ${governor.reason}`,
        });
        setLastExecution({
            message: `Swap blocked: ${governor.reason}`,
        });
        return (0, exports.getAgentStatus)();
    }
    (0, riskEngine_1.assertTradeAllowed)({
        protocol: "jupiter_swap",
        amountSol: context.amountSol,
        slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
    });
    try {
        const execution = await jupiterSwapModule_1.JupiterSwapModule.executeSwap({
            walletPublicKey: context.topology.traderWalletPublicKey,
            inputMint: context.intent.direction === "SOL_TO_USDC" ? WSOL_MINT : env_1.env.defaultUsdcMint,
            outputMint: context.intent.direction === "SOL_TO_USDC" ? env_1.env.defaultUsdcMint : WSOL_MINT,
            amount: context.intent.direction === "SOL_TO_USDC"
                ? Math.floor(context.amountSol * web3_js_1.LAMPORTS_PER_SOL)
                : Math.floor(context.notionalUsdc * 10 ** USDC_DECIMALS),
            slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
            maxAllowedSlippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
            reasoningReference: context.intent.reason,
        });
        runtime.swapStats.lastSwapAt = new Date().toISOString();
        runtime.swapStats.dailySwapCount += 1;
        runtime.swapStats.successfulSwaps += 1;
        runtime.swapStats.consecutiveSwapFailures = 0;
        await (0, splEconomyEngine_1.handleTraderSwapOutcome)({
            topology: context.topology,
            successful: true,
            profitable: execution.slippageBps <= (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps * 0.6,
            notionalUsdc: context.notionalUsdc,
        });
        await (0, splEconomyEngine_1.maybeMintActivityRewards)(context.topology);
        await (0, splEconomyEngine_1.maybeRedistributeFromTreasury)(context.topology);
        (0, riskEngine_1.recordExecutionSuccess)();
        runtime.cycles += 1;
        setLastExecution({
            message: `Jupiter swap executed (${context.intent.direction})`,
            txSignature: execution.signature,
            explorerUrl: execution.explorerUrl,
        });
        (0, websocket_1.emitLog)({
            level: "success",
            message: `Swap executed: ${context.intent.direction}`,
            txSignature: execution.signature,
            explorerUrl: execution.explorerUrl,
            data: {
                route: execution.route,
                inAmount: execution.inAmount,
                outAmount: execution.outAmount,
                reasoning: context.intent.reason,
            },
        });
        return (0, exports.getAgentStatus)();
    }
    catch (error) {
        runtime.swapStats.failedSwaps += 1;
        runtime.swapStats.consecutiveSwapFailures += 1;
        runtime.swapStats.lastSwapAt = new Date().toISOString();
        await (0, splEconomyEngine_1.handleTraderSwapOutcome)({
            topology: context.topology,
            successful: false,
            profitable: false,
            notionalUsdc: context.notionalUsdc,
        });
        (0, riskEngine_1.recordExecutionFailure)(error);
        runtime.failures += 1;
        runtime.lastRunAt = new Date().toISOString();
        runtime.lastAction = "Execution failed";
        (0, websocket_1.emitLog)({
            level: "error",
            message: error instanceof Error ? error.message : String(error),
        });
        throw error;
    }
};
const cycle = async () => {
    if (runtime.cycleInProgress) {
        return (0, exports.getAgentStatus)();
    }
    runtime.cycleInProgress = true;
    let statusAfterRun = null;
    try {
        (0, websocket_1.emitLog)({
            level: "info",
            message: "Agent cycle started",
            data: {
                profile: "deterministic_jupiter_strategy",
            },
        });
        statusAfterRun = await runDeterministicCycle();
    }
    finally {
        runtime.cycleInProgress = false;
    }
    return statusAfterRun ?? (0, exports.getAgentStatus)();
};
const runAgent = async () => cycle();
exports.runAgent = runAgent;
const startAgent = (intervalMs = env_1.env.defaultLoopMs) => {
    if (runtime.running) {
        return (0, exports.getAgentStatus)();
    }
    runtime.running = true;
    runtime.intervalMs = intervalMs;
    (0, websocket_1.emitLog)({
        level: "info",
        message: `Autonomous loop started (${intervalMs}ms interval)`,
    });
    void cycle().catch(() => undefined);
    runtime.timer = setInterval(() => {
        void cycle().catch(() => undefined);
    }, intervalMs);
    return (0, exports.getAgentStatus)();
};
exports.startAgent = startAgent;
const stopAgent = () => {
    if (runtime.timer) {
        clearInterval(runtime.timer);
        runtime.timer = null;
    }
    runtime.running = false;
    (0, websocket_1.emitLog)({
        level: "warn",
        message: "Autonomous loop paused",
    });
    return (0, exports.getAgentStatus)();
};
exports.stopAgent = stopAgent;
const executeTrade = async (amountSol = 0.1) => {
    if (!env_1.env.enableJupiterSwap) {
        throw new Error("Jupiter execution is disabled. Enable ENABLE_JUPITER_SWAP=true and use devnet balances.");
    }
    const topology = await ensureTopology();
    const walletState = await (0, walletManager_1.getWalletState)();
    const traderWallet = walletState.wallets.find((wallet) => wallet.publicKey === topology.traderWalletPublicKey);
    if (!traderWallet) {
        throw new Error("Trader wallet is missing from wallet state.");
    }
    if (!Number.isFinite(amountSol) || amountSol <= 0) {
        throw new Error("amountSol must be greater than 0");
    }
    const maxSwapPct = traderWallet.solBalance > 0 ? Number((amountSol / traderWallet.solBalance).toFixed(6)) : 1;
    const totalTrades = runtime.swapStats.successfulSwaps + runtime.swapStats.failedSwaps;
    const successRate = totalTrades === 0 ? 1 : runtime.swapStats.successfulSwaps / totalTrades;
    const riskScore = Number((runtime.swapStats.consecutiveSwapFailures * 18 +
        (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps / 5 +
        (traderWallet.solBalance < env_1.env.jupiterMinSolReserve ? 25 : 0) +
        (1 - successRate) * 20).toFixed(2));
    const governor = (0, jupiterGovernor_1.evaluateJupiterGovernor)({
        solBalance: traderWallet.solBalance,
        requestedSwapSol: amountSol,
        requestedSwapPct: maxSwapPct,
        slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        maxAllowedSlippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
        dailySwapCount: runtime.swapStats.dailySwapCount,
        riskScore,
    });
    if (!governor.allowed) {
        const blocked = `Swap blocked: ${governor.reason}`;
        (0, websocket_1.emitLog)({
            level: "warn",
            message: blocked,
            data: { amountSol, riskScore },
        });
        setLastExecution({ message: blocked });
        throw new Error(blocked);
    }
    (0, riskEngine_1.assertTradeAllowed)({
        protocol: "jupiter_swap",
        amountSol,
        slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
    });
    const reasoningReference = `Manual executeTrade invocation (${amountSol} SOL)`;
    try {
        const execution = await jupiterSwapModule_1.JupiterSwapModule.executeSwap({
            walletPublicKey: topology.traderWalletPublicKey,
            inputMint: WSOL_MINT,
            outputMint: env_1.env.defaultUsdcMint,
            amount: Math.floor(amountSol * web3_js_1.LAMPORTS_PER_SOL),
            slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
            maxAllowedSlippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
            reasoningReference,
        });
        runtime.swapStats.lastSwapAt = new Date().toISOString();
        runtime.swapStats.dailySwapCount += 1;
        runtime.swapStats.successfulSwaps += 1;
        runtime.swapStats.consecutiveSwapFailures = 0;
        runtime.cycles += 1;
        runtime.lastDecision = {
            action: "SELL",
            protocol: "jupiter_swap",
            inputMint: WSOL_MINT,
            outputMint: env_1.env.defaultUsdcMint,
            amountSol,
            slippageBps: (0, riskEngine_1.getRiskStatus)().settings.maxSlippageBps,
            confidence: 100,
            reason: reasoningReference,
        };
        runtime.lastIntent = {
            action: "SWAP",
            direction: "SOL_TO_USDC",
            amountPct: maxSwapPct,
            confidence: 100,
            reason: reasoningReference,
        };
        await (0, splEconomyEngine_1.handleTraderSwapOutcome)({
            topology,
            successful: true,
            profitable: true,
            notionalUsdc: amountSol * (await estimateSolPriceUsdc()),
        });
        await (0, splEconomyEngine_1.maybeMintActivityRewards)(topology);
        await (0, splEconomyEngine_1.maybeRedistributeFromTreasury)(topology);
        (0, riskEngine_1.recordExecutionSuccess)();
        setLastExecution({
            message: `Manual trade executed (${amountSol} SOL)`,
            txSignature: execution.signature,
            explorerUrl: execution.explorerUrl,
        });
        return {
            success: true,
            decision: runtime.lastDecision,
            txSignature: execution.signature,
            explorerUrl: execution.explorerUrl,
        };
    }
    catch (error) {
        runtime.swapStats.failedSwaps += 1;
        runtime.swapStats.consecutiveSwapFailures += 1;
        runtime.swapStats.lastSwapAt = new Date().toISOString();
        runtime.failures += 1;
        (0, riskEngine_1.recordExecutionFailure)(error);
        (0, websocket_1.emitLog)({
            level: "error",
            message: error instanceof Error ? error.message : String(error),
            data: {
                source: "manual_execute_trade",
                amountSol,
            },
        });
        throw error;
    }
};
exports.executeTrade = executeTrade;
const runSimulation = async () => {
    const context = await buildIntentContext();
    const decision = mapIntentToDecision(context.intent, context.amountSol);
    const llmReference = await (0, decisionEngine_1.getLLMDecision)({
        profile: "jupiter_simulation_reference",
        intent: context.intent,
    }).catch(() => decision);
    (0, websocket_1.emitLog)({
        level: "info",
        message: `Simulation: ${context.intent.action} ${context.intent.direction ?? ""}`,
        data: {
            deterministicIntent: context.intent,
            llmReference,
        },
    });
    return {
        success: true,
        decision,
        intent: context.intent,
        risk: (0, riskEngine_1.getRiskStatus)(),
        splEconomy: (0, splEconomyEngine_1.getSplEconomyStatus)(),
    };
};
exports.runSimulation = runSimulation;
const getAgentStatus = () => ({
    running: runtime.running,
    cycleInProgress: runtime.cycleInProgress,
    intervalMs: runtime.intervalMs,
    cycles: runtime.cycles,
    failures: runtime.failures,
    lastRunAt: runtime.lastRunAt,
    lastAction: runtime.lastAction,
    lastDecision: runtime.lastDecision,
    lastIntent: runtime.lastIntent,
    lastTxSignature: runtime.lastTxSignature,
    lastExplorerUrl: runtime.lastExplorerUrl,
    risk: (0, riskEngine_1.getRiskStatus)(),
    jupiterSwapStats: { ...runtime.swapStats },
    topology: runtime.topology,
    splEconomy: (0, splEconomyEngine_1.getSplEconomyStatus)(),
});
exports.getAgentStatus = getAgentStatus;
