...
exports.toClientError = void 0;
const toClientError = (error) => {
    const fromErrorMessage = error instanceof Error ? error.message : String(error ?? "");
    const extended = error;
    const raw = (fromErrorMessage ||
    ...
    if (lower.includes("attempt to debit an account but found no record of a prior credit")) {
    ...
    if (lower.includes("address table account") &&
